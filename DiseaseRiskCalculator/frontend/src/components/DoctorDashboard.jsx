...
function DoctorDashboard() {
  ...
  const fetchGenes = async () => {
    try {
      const response = await axios.get('https://165.22.244.125:8000/api/genes/my_genes/', {
        headers: {
  ...
  useEffect(() => {
    const fetchFormula = async () => {
      try {
        const response = await axios.get('https://165.22.244.125:8000/api/formulas/', {
          headers: {
  ...
  useEffect(() => {
    const refreshAccessToken = async () => {
      try {
        const response = await axios.post('https://165.22.244.125:8000/api/token/refresh/', {
  ...
  const handleSaveGene = async (geneData) => {
    try {
      await axios.post(
        'https://165.22.244.125:8000/api/genes/edit_gene/',
        {
          gene_id: geneData.id,
          name: geneData.name,
  ...
  const handleUploadGene = async (formData) => {
    try {
      await axios.post('https://165.22.244.125:8000/api/genes/upload/', formData, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
  ...
  const handleDeleteGene = async () => {
    try {
      await axios.delete(`https://165.22.244.125:8000/api/genes/delete_gene/`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveFormula = async (newFormula) => {
    try {
      await axios.post('https://165.22.244.125:8000/api/formulas/', {
        formula: newFormula,
      }, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
  ...
  return (
    <div className="doctor-dashboard">
      ...
      <h3>Formula Data</h3>
      {/* Make two columns, left for the formula and right for update formula button */}
      <div className="formula-container">
        <div className="formula-text">
          <h4>Formula:</h4>
          <p>{formula}</p>
        </div>
        <button className="update-formula-btn" onClick={() => setShowUpdateFormulaModal(true)}>Update Formula</button>
      </div>
